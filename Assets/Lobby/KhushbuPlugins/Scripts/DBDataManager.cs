...
{
    public class DBDataManager<T> where T : new()
    {
        private T stats;
        private string key;

        public DBDataManager(string filename)
        {
            this.key = filename;
            stats = Load();
        }
        ...
        private T Load()
        {
            if (!DBPlayerPrefs.HasKey(key))
                return new T();

            string data = DBPlayerPrefs.GetString(key);
        ...
        public void Save(T stats)
        {
            ...
            DBPlayerPrefs.SetString(key, serializedData);
            DBPlayerPrefs.Save();
        }
